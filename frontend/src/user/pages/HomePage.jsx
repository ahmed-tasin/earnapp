...
function HomePage() {
  ...
  const handleDailyCheckin = async () => {
    try {
      ...
      setMessage(response.data?.message || "Daily check-in successful");
  ...
  const getInitial = () => {
    return dashboard.user?.name?.charAt(0)?.toUpperCase() || "U";
  };
      ...
      <header className="user-home-header">
        <div className="user-header-profile">
          {dashboard.user.profilePicture ? (
            ...
            <div className="user-avatar-placeholder">{getInitial()}</div>
          )}

          <div>
            <span className="user-welcome-text">Welcome back</span>
      ...
      <section className="wallet-balance-card">
        <div className="wallet-card-top">
          <div>
            ...
            <h2>৳{formatMoney(dashboard.wallet.balance)}</h2>
          </div>
      ...
      <section className="home-stat-grid">
        <div className="home-stat-card">
          ...
          <div>
            ...
            <strong>৳{formatMoney(dashboard.wallet.totalDeposit)}</strong>
          </div>
        </div>
        ...
        <div className="home-stat-card">
          ...
          <div>
            ...
            <strong>৳{formatMoney(dashboard.wallet.totalEarning)}</strong>
          </div>
        </div>
        ...
        <div className="home-stat-card">
          ...
          <div>
            ...
            <strong>৳{formatMoney(dashboard.wallet.totalWithdraw)}</strong>
          </div>
        </div>
      ...
      <section className="daily-checkin-card">
        <div>
          ...
          <div>
            ...
            <p>Check in every day and collect your reward.</p>
          </div>
        </div>
      ...
      <section className="home-section">
        ...
        {dashboard.activeInvestment ? (
          <div className="active-investment-card">
            <div className="investment-card-header">
              <div>
                ...
                <h3>
                  {dashboard.activeInvestment.packageId?.name ||
                    dashboard.activeInvestment.package?.name ||
                    dashboard.activeInvestment.packageName ||
                    "Investment Package"}
                </h3>
              </div>
              ...
              <span
                className={`investment-status ${
                  dashboard.activeInvestment.status || "active"
                }`}
              >
                {dashboard.activeInvestment.status || "active"}
              </span>
            ...
            <div className="investment-details-grid">
              <div>
                ...
                <strong>
                  ৳
                  {formatMoney(
                    dashboard.activeInvestment.investmentAmount ||
                      dashboard.activeInvestment.amount,
                  )}
                </strong>
              </div>
              ...
              <div>
                ...
                <strong>
                  ৳{formatMoney(dashboard.activeInvestment.dailyReturn)}
                </strong>
              </div>
              ...
              <div>
                ...
                <strong>{dashboard.activeInvestment.remainingDays ?? 0}</strong>
              </div>
            </div>
          </div>
        ) : (
          <div className="no-investment-card">
            ...
            <p>Choose a package to start earning.</p>
